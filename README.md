# ArtSplat: Feed-Forward Articulated 3D Gaussian Splatting from Sparse Multi-State Uncalibrated Views

**[Project Page](https://artsplat.github.io/)** · arXiv (coming soon) · 🤗 Hugging Face (coming soon)

Inseo Lee¹, Yoonji Kim², Eugene Sohn¹, Jiwoong Lee¹, Jungmin You¹, Joonseok Lee¹†, Jin-Hwa Kim¹³†

¹ Seoul National University &nbsp;·&nbsp; ² Sogang University &nbsp;·&nbsp; ³ NAVER AI Lab &nbsp;·&nbsp; † Co-corresponding authors

---

ArtSplat reconstructs both **geometry** and **joint parameters** of an articulated object from sparse, uncalibrated RGB views across multiple articulation states — in a single feed-forward pass, over **400× faster** than per-object optimization baselines.

<!-- Optional: hero figure -->
<!-- <p align="center"><img src="docs/teaser.png" width="80%"></p> -->

## Status

🚧 **Code release coming soon.** We are cleaning up training and inference code for public release. Watch / star this repo to be notified.

Planned release:
- [ ] Inference code + pretrained checkpoints
- [ ] Training pipeline
- [ ] Hugging Face demo

## Citation

```bibtex
@article{lee2026artsplat,
  title   = {ArtSplat: Feed-Forward Articulated 3D Gaussian Splatting from Sparse Multi-State Uncalibrated Views},
  author  = {Lee, Inseo and Kim, Yoonji and Sohn, Eugene and Lee, Jiwoong and You, Jungmin and Lee, Joonseok and Kim, Jin-Hwa},
  journal = {arXiv preprint arXiv:XXXX.XXXXX},
  year    = {2026}
}
```

## License

To be determined upon code release.

